function dispStruct = initIOStruct(taskStruct, stimDir)
    ...
    %debugWinSize = [0,0,600,200]; % this allows me to run the task in debug mode
    fullWinSize = [];
    %run full-screen task
    %[dispStruct.wPtr, dispStruct.wPtrRect] = Screen('OpenWindow', 0, dispStruct.bgColor, debugWinSize); %fullWinSize); % allows me to run task in debug mode
    [dispStruct.wPtr, dispStruct.wPtrRect] = Screen('OpenWindow', 0, dispStruct.bgColor, fullWinSize); %fullWinSize);
    ...
    rewardHeight = machineImageHeight * 0.2; % made bigger so the pic wouldn't get cut-off. Not sure how to center letters though.
    ...
    width = 300; height = 225; % manipulate to change size of creatures
    %width = 300; height = 225;
