...
## Model: Reward by age, trial and block difficulty
```{r trialwise reward by age and block difficulty}

#select variables and filter
trialwise_reward <- exploAll %>%
    select(age, blockDifficulty, blockID, trialID, subID, reward) %>%
    drop_na()

#scale variables
trialwise_reward$ageScaled <- scale_this(trialwise_reward$age)
trialwise_reward$trialScaled <- scale_this(trialwise_reward$trialID)


trialwise_reward.model <- mixed(reward ~ ageScaled * blockDifficulty * trialScaled + 
                                 (trialScaled || subID), 
                             data = trialwise_reward,
                             family = "binomial",
                             method = "LRT",
...
#show results
trialwise_reward.model

# Note: singular fit w/ interaction in RE
# Still singular fit w/o interaction - removed block difficulty from RE
...
## Model: Reward by age, trial, block difficulty, and block number
```{r trialwise reward by age block difficulty and block number}

#scale variables
trialwise_reward$blockScaled <- scale_this(trialwise_reward$blockID)

trialwise_reward.block.model <- mixed(reward ~ ageScaled * blockDifficulty * trialScaled * blockScaled + 
                                 (trialScaled || subID), 
                             data = trialwise_reward,
                             family = "binomial",
                             method = "LRT",
...
#show results
trialwise_reward.block.model

# Note: singular fit w/ interaction in RE
# Still singular fit - removed block difficulty from RE
```

## Odds ratios for model: Reward by age, trial, block difficulty, and block number
```{r trialwise reward by age block difficulty and block number odds ratios}

#scale variables
trialwise_reward.block.glmer <- mixed(reward ~ ageScaled * blockDifficulty * trialScaled * blockScaled + 
                                 (trialScaled || subID), 
                             data = trialwise_reward,
                             family = "binomial",
                             method = "LRT",
...
#show results with odds ratios
tidy(trialwise_reward.block.glmer,
     conf.int=TRUE,
     exponentiate=TRUE,
     conf.level = 0.95,
     effects="fixed")
...
opt_choices.filtered <- opt_choices %>%
    select(opt_choice, age, blockDifficulty, trialID, subID, blockID) %>%
    drop_na()
...
opt_choices.filtered$blockScaled <- scale_this(opt_choices.filtered$blockID)
...
## Model: Optimal choices by age, block difficulty, and block number
```{r optimal choices by age block difficulty block number}

optChoice.age.block.model <- mixed(opt_choice ~ ageScaled * blockDifficulty * trialScaled *blockScaled + 
                                 (blockDifficulty + trialScaled + blockScaled | subID), 
                             data = opt_choices.filtered,
                             family = "binomial",
                             method = "LRT",
                             control = glmerControl(optimizer = "bobyqa"),
...
#show results
optChoice.age.block.model

# main effect of age
# main effect of block difficulty
# main effect of trial
# block number x block difficulty
# age x trial x block number

```

## Odds ratios for model: Optimal choices by age, block difficulty, and block number
```{r optimal choices by age block difficulty block number odds ratios}

optChoice.age.block.glmer <- mixed(opt_choice ~ ageScaled * blockDifficulty * trialScaled *blockScaled + 
                                 (blockDifficulty + trialScaled + blockScaled | subID), 
                             data = opt_choices.filtered,
                             family = "binomial",
                             method = "LRT",
...
tidy(optChoice.age.block.glmer,
     conf.int=TRUE,
     exponentiate=TRUE,
     conf.level = 0.95,
     effects="fixed")
...
## Plot: Optimal choices by  block difficulty and block number
```{r opt choice age block number plot }
mean_opt_choice <- opt_choices %>%
    mutate(blockHalf = case_when(blockID < 6 ~ 1,
                                 blockID > 5 ~ 2)) %>%
    group_by(subID, blockDifficulty, blockID) %>%
    summarize(meanOptChoice = mean(opt_choice, na.rm = T))

opt_choice_block_num_plot <- ggplot(mean_opt_choice, aes(x = blockID, y = meanOptChoice, color = blockDifficulty, fill = blockDifficulty)) +
    geom_point() +
    geom_smooth(method = "lm") + 
    scale_color_manual(values = c(color1, color2), name = "Block Difficulty", labels = c("Easy", "Hard")) +
    scale_fill_manual(values = c(color1, color2), name = "Block Difficulty", labels = c("Easy", "Hard")) +
...
#compute correlation between novelty and uncertainty
novUncertainCorr <- cor.test(data.model$varDiff, data.model$novelDiff)
...
#save data for hddm
#write_csv(data.model, file = "processed_choice_data.csv")
...
```{r feature plot with features as facets, fig.width=8, fig.height=4}
#combine means
...
feature_plot <- ggplot(feature_means, aes(x = evDiffGroup, y = meanChoice, color = ageGroup3)) +
    ...
    geom_line(aes(group = ageGroup3), size = .5) + 
...
## Odds ratios for model: Influence of age, value, uncertainty, and novelty on choice
```{r choice features model age odds ratios}

feature.age.glmer <- mixed(leftChoice ~ 
                               evDiffScaled + 
                               varDiffScaled + 
                               novelDiffScaled +
                               evDiffScaled:ageScaled +
...
## Model: Influence of trial, value, uncertainty, novelty on choice - kids only
```{r choice features model: kids only}
...
#run model
feature.children.model = mixed(leftChoice ~ 
                                    evDiffScaled + 
                                    varDiffScaled + 
                                    novelDiffScaled +
                                    (evDiffScaled + varDiffScaled + novelDiffScaled || subID),
                   ...
                   control = glmerControl(optimizer = "bobyqa"),
                   expand_re = T)
feature.children.model
```

## Odds ratios: Influence of trial, value, uncertainty, novelty on choice - kids only
```{r OR kids only model}
#run model
feature.children.glmer = mixed(leftChoice ~ 
                                    evDiffScaled + 
                                    varDiffScaled + 
                                    novelDiffScaled +
                                    (evDiffScaled + varDiffScaled + novelDiffScaled || subID),
                   ...
                   expand_re = T, 
                   return = "merMod")
feature.children.glmer

#show results
tidy(feature.children.glmer,
     conf.int=TRUE,
     exponentiate=TRUE,
     conf.level = 0.95,
     effects="fixed")
...
## Model: Influence of block number, age, value, uncertainty, and novelty on choice
```{r choice features model with block number}

#select variables
data.model.1 <- data.model %>%
    select(c(leftChoice, evDiff, varDiff, novelDiff, age, subID, blockID)) %>%
    drop_na()
...
data.model.1$novelDiffScaled = scale_this(data.model.1$novelDiff)
data.model.1$ageScaled <- scale_this(data.model.1$age)
data.model.1$blockScaled <- scale_this(data.model.1$blockID)

feature.age.block.model <- mixed(leftChoice ~ 
                               evDiffScaled + 
                               varDiffScaled + 
                               novelDiffScaled +
                               evDiffScaled:ageScaled +
                               varDiffScaled:ageScaled + 
                               novelDiffScaled:ageScaled +
                               evDiffScaled:blockScaled +
                               varDiffScaled:blockScaled + 
                               novelDiffScaled:blockScaled + 
                               evDiffScaled:ageScaled:blockScaled +
                               varDiffScaled:ageScaled:blockScaled + 
                               novelDiffScaled:ageScaled:blockScaled +
                               (evDiffScaled + varDiffScaled + novelDiffScaled || subID),
                   data.model.1, 
                   family = binomial, 
                   method = "LRT",
                   control = glmerControl(optimizer = "bobyqa"),
                   expand_re = T)
feature.age.block.model 


```


## Odds ratios: Influence of block number, age, value, uncertainty, and novelty on choice
```{r OR choice features with block number}
#run model
feature.age.block.glmer <- mixed(leftChoice ~ 
                               evDiffScaled + 
                               varDiffScaled + 
                               novelDiffScaled +
                               evDiffScaled:ageScaled +
                   ...
                   expand_re = T,
                   return = "merMod")
feature.age.block.glmer 

#show results
tidy(feature.age.block.glmer,
     conf.int=TRUE,
     exponentiate=TRUE,
     conf.level = 0.95,
     effects="fixed")
...
## Odds ratios: Influence of age, IQ, value, uncertainty, and novelty on choice
```{r OR feature age IQ}

#run model
feature.age.IQ.glmer <- mixed(leftChoice ~ 
                               evDiffScaled + 
                               varDiffScaled + 
                               novelDiffScaled +
                               evDiffScaled:ageScaled +
                   ...
                   expand_re = T,
                   return = "merMod")
feature.age.IQ.glmer

#show results
tidy(feature.age.IQ.glmer,
     conf.int=TRUE,
     exponentiate=TRUE,
     conf.level = 0.95,
     effects="fixed")
...
## Odds ratios: Influence of age, trial, value, uncertainty, and novelty on choice
```{r odds ratio for choice features model with trial number}

#run glmer
feature.age.trial.glmer = mixed(leftChoice ~ 
                                    evDiffScaled + 
                                    varDiffScaled + 
                                    novelDiffScaled +
                                    evDiffScaled:scaledTrialNum +
...
## Model: Influence of trial, value, uncertainty, novelty, and trial on choice - kids only
...
## Odds ratios: Influence of age, trial, value, uncertainty, and novelty on choice (children only)
```{r odds ratio for choice features model with trial number (children only)}

#run glmer
feature.children.trial.glmer = mixed(leftChoice ~ 
                                    evDiffScaled + 
                                    varDiffScaled + 
                                    novelDiffScaled +
                                    evDiffScaled:scaledTrialNum +
...
#show results with odds ratios
tidy(feature.children.trial.glmer,
     conf.int=TRUE,
     exponentiate=TRUE,
     conf.level = 0.95,
     effects="fixed")
...
## Odds ratios: Influence of age, quadratic age, value, uncertainty, and novelty on choice 
```{r odds ratio for choice features model with quadratic age}

#run glmer
feature.quad_age.glmer <- mixed(leftChoice ~ 
                               evDiffScaled + 
                               varDiffScaled + 
                               novelDiffScaled +
                               evDiffScaled:ageScaled +
...
## Plot: Influence of learning features on choice behavior (four age groups)
```{r feature plot with age group as facets (four age groups), fig.width=8, fig.height=6}
#combine means
feature_means2 <- rbind(value_means2, uncertainty_means2, novel_means2)

feature_plot_ageFacets2 <- ggplot(feature_means2, aes(x = evDiffGroup, y = meanChoice, color = group)) +
    facet_wrap(~ageGroup4) +
    geom_point(size = 2) +
    geom_errorbar(aes(ymin = meanChoice - seChoice, ymax = meanChoice + seChoice), width = 0) +
    scale_color_manual(values = c(color3, color1, color2), name = "",
                       labels = c("Left vs. Right",
...
## Odds ratios: Influence of age, quadratic age, IQ, value, uncertainty, and novelty on choice 
```{r odds ratio for choice features model with quadratic age and IQ}

#run glmer
feature.quad_age.IQ.glmer = mixed(leftChoice ~ 
                                    evDiffScaled + 
                                    varDiffScaled + 
                                    novelDiffScaled +
                                    evDiffScaled:ageScaled + 
...
#show results with odds ratios
tidy(feature.quad_age.IQ.glmer,
     conf.int=TRUE,
     exponentiate=TRUE,
     conf.level = 0.95,
     effects="fixed")
...
```{r odds ratios for evDiffNoReset model}
#scale variables
feature.age.no_reset_EV.glmer <- mixed(leftChoice ~ 
                               evDiffScaled + 
                               varDiffScaled + 
                               novelDiffScaled +
                               evDiffNoResetScaled + 
...
#show results
tidy(feature.age.no_reset_EV.glmer,
     conf.int=TRUE,
     exponentiate=TRUE,
     conf.level = 0.95,
     effects="fixed")
...
# Influence of choice features on RT across age
## RT distributions
```{r plot untransformed and log-transformed RTs}
...
#un-transformed
hist(data.model$RT,
     xlab = "Response Times (seconds)",
     ylab = "Number of Responses")

#transformed
hist(data.model$logRT,
     xlab = "Log Response Times (seconds)",
     ylab = "Number of Responses")
```
...
data.model.8$logRT <- log(data.model.8$RT)
...
#display estimates
summary(selectedUncertainty.age.model.RT)
```

```{r selectedUncertainty RT model lmer}

data.model.8a <- data.model.8
data.model.8a$nov <- data.model.8a$selectedNovelty
data.model.8a$var <- data.model.8a$selectedUncertainty
data.model.8a$exv <- data.model.8a$selectedValue

selectedUncertainty.age.RT.lmer <- mixed(
    logRT ~
        ageScaled:nov +
        ageScaled:var +
        ageScaled:exv +
...
```{r selected uncertainty RT model interaction plot}
...
RT.int.plot <- plot_model(selectedUncertainty.age.RT.lmer, 
                          type = "int",
                          mdrt.values = "meansd",
                          title = "",
                          colors = c(color1, color2, color3),
                          axis.title = c("Age (z score)", "Log RT"),
                          ci.lvl = .95) 
RT.int.plot 
```


```{r selectedUncertainty RT model lmer check residuals}

selectedUncertainty.age.RT.lmer <- mixed(
    logRT ~
        ageScaled:selectedNovelty +
        ageScaled:selectedUncertainty +
        selectedUncertainty +
        selectedValue +
        selectedValue:ageScaled +
        selectedNovelty +
        ageScaled +
        (selectedUncertainty + selectedValue + selectedNovelty |
             subID),
    data.model.8,
    method = "S",
    control = lmerControl(optimizer = "bobyqa"),
...
# QQ plot
qqnorm(residuals(selectedUncertainty.age.RT.lmer))

#histogram of residuals
hist(residuals(selectedUncertainty.age.RT.lmer),
...
summary(selectedUncertainty.age.model.trial.RT)
```
...
summary(selectedUncertainty.model.logRT.children)
...
## Model: Influence of value, novelty, and uncertainty of selected option on RT in adolescents
```{r RT by features of selected option - adolescents only}

#select data
data.model.10a <- data.model %>%
    filter(ageGroup3 == "Adolescents") %>%
    select(c(RT, selectedUncertainty, selectedValue, selectedNovelty,
             age, subID)) %>%
    drop_na()
...
#scale continuous variables
data.model.10a$selectedUncertainty <- scale_this(data.model.10a$selectedUncertainty)
data.model.10a$selectedValue <- scale_this(data.model.10a$selectedValue)
data.model.10a$selectedNovelty <- scale_this(data.model.10a$selectedNovelty)
data.model.10a$ageScaled <- scale_this(data.model.10a$age)
data.model.10a$logRT <- log(data.model.10a$RT)


selectedUncertainty.model.logRT.adolescents <- mixed(logRT ~ 
                                    selectedUncertainty +
                                    selectedValue +
                                    selectedNovelty +
                                                                            selectedUncertainty:ageScaled +
                   ...
                   control = lmerControl(optimizer = "bobyqa"),
                   expand_re = T)
selectedUncertainty.model.logRT.adolescents

summary(selectedUncertainty.model.logRT.adolescents)
...
## Model: Influence of value, novelty, and uncertainty of selected option on RT in adults
```{r RT by features of selected option - adults only}

#select data
data.model.10b <- data.model %>%
    filter(ageGroup3 == "Adults") %>%
    select(c(RT, selectedUncertainty, selectedValue, selectedNovelty,
             age, subID)) %>%
    drop_na()
...
#scale continuous variables
data.model.10b$selectedUncertainty <- scale_this(data.model.10b$selectedUncertainty)
data.model.10b$selectedValue <- scale_this(data.model.10b$selectedValue)
data.model.10b$selectedNovelty <- scale_this(data.model.10b$selectedNovelty)
data.model.10b$ageScaled <- scale_this(data.model.10b$age)
data.model.10b$logRT <- log(data.model.10b$RT)


selectedUncertainty.model.logRT.adults <- mixed(logRT ~ 
                                    selectedUncertainty +
                                    selectedValue +
                                    selectedNovelty +
                                                                            selectedUncertainty:ageScaled +
                   ...
                   control = lmerControl(optimizer = "bobyqa"),
                   expand_re = T)
selectedUncertainty.model.logRT.adults

summary(selectedUncertainty.model.logRT.adults)
...
summary(selectedUncertainty.model.logRT.trial.children)
...
# Parameter estimates from familiarity-gated uncertainty model
```{r fg params}
fg_params <- read_csv(file.path(dataDir, "model_output/params_uL_fG.csv"))

#combine with demographic data
fg_params <- full_join(fg_params, demographicData, by = c("subID"))
```

## Relation between model parameter estimates from familiarity-gated uncertainty model and memory
```{r fg params and memory}

#combine with memory accuracy
fg_params_mem <- full_join(fg_params, sub_mem_acc, by = c("subID")) %>%
    filter(ageGroup3 %in% c("Adolescents", "Adults")) %>%
    select(subID, age, ageGroup3, smB, rlP, uL, mean_mem_acc)

#scale age and params
fg_params_mem$ageScaled <- scale_this(fg_params_mem$age)
fg_params_mem$smBScaled <- scale_this(fg_params_mem$smB)
fg_params_mem$rlPScaled <- scale_this(fg_params_mem$rlP)
fg_params_mem$uLScaled <- scale_this(fg_params_mem$uL)

#run regression
fg.mem.model <- lm(mean_mem_acc ~ ageScaled * (uLScaled + rlPScaled + smBScaled), data = fg_params_mem)
summary(fg.mem.model)
#no effects
```


# Parameter estimates from novelty model
```{r nl params}
nl_params <- read_csv(file.path(dataDir, "model_output/params_nL.csv"))

#combine with demographic data
nl_params <- full_join(nl_params, demographicData, by = c("subID"))
```


## Relation between model parameter estimates from novelty model and memory
```{r novelty params and memory}

#combine with memory accuracy
nL_params_mem <- full_join(nl_params, sub_mem_acc, by = c("subID")) %>%
    filter(ageGroup3 %in% c("Children")) %>%
    select(subID, age, ageGroup3, smB, rlP, nL, mean_mem_acc)

#scale age and params
nL_params_mem$ageScaled <- scale_this(nL_params_mem$age)
nL_params_mem$smBScaled <- scale_this(nL_params_mem$smB)
nL_params_mem$rlPScaled <- scale_this(nL_params_mem$rlP)
nL_params_mem$nLScaled <- scale_this(nL_params_mem$nL)

#run regression
nL.mem.model <- lm(mean_mem_acc ~ ageScaled * (smBScaled + rlPScaled + nLScaled), data = nL_params_mem)
summary(nL.mem.model)
#main effect of age
```

# Parameter estimates from novelty and uncertainty model
```{r nL uL params}
model_params <- read_csv(file.path(dataDir, "model_output/params_nL_uL.csv"))

#combine with demographic data
model_params <- full_join(model_params, demographicData, by = c("subID"))
```

## Relation between parameter estimates from novelty + uncertainty model and age
```{r nLuL uncertainty param and age}

uncertainty.age.model <- lm(uL ~ age, data = model_params)
summary(uncertainty.age.model)

#significant effect
```

```{r nLuL novelty param and age}

novelty.age.model <- lm(nL ~ age, data = model_params)
summary(novelty.age.model)

#no significant effect
```

```{r nLuL beta param and age}
beta.age.model <- lm(smB ~ age, data = model_params)
summary(beta.age.model)

#significant effect
```

```{r nLuL learning rate and age}
learning_rate.age.model <- lm(rlP ~ age, data = model_params)
summary(learning_rate.age.model)
